...
// Add these methods to your ConsumptionController.php after the exportMonthlyBreakdownPDF method

public function yearlyBreakdownView()
{
   
    return view('consumption.yearly-breakdown');
}

public function getYearlyBreakdownData(Request $request)
{
    $request->validate([
    ...
    try {
        ...
        for ($month = 1; $month <= 12; $month++) {
            ...
            // Get monthly data using existing method
        ...
        Log::error('Error fetching yearly breakdown data: ' . $e->getMessage());
        Log::error($e->getTraceAsString());
        return response()->json([
            'status' => 500,
            'message' => 'Erreur lors de la récupération des données: ' . $e->getMessage()
...
public function exportYearlyBreakdownPDF(Request $request)
{
    $request->validate([
        'year' => 'required|integer|min:2020|max:2100',
        'type_menu' => 'required|string',
    ...
    try {
        ...
        // Get the data directly from the existing method
        $response = $this->getYearlyBreakdownData($request);
        
        // Parse the JSON response
        $responseData = json_decode($response->getContent(), true);
        
        // Check if we have data
        if ($responseData['status'] !== 200 || empty($responseData['data']['months_data'])) {
            return response()->json([
                'status' => 404,
                'message' => 'Aucune donnée trouvée pour cette année et ces critères'
            ]);
        ...
        // Generate PDF using the template
        $pdf = Pdf::loadView('consumption.yearly_breakdown_pdf', [
            'data' => $responseData['data'],
            'year' => $year,
            'type_menu' => $typeMenu,
            'imageData_top' => $imageData_top,
        ...
        // Generate filename that includes menu type
        $filename = 'reporting_annuel_' . $year;
        if ($typeMenu && $typeMenu !== 'all') {
            $filename .= '_' . str_replace(' ', '_', strtolower($typeMenu));
        }
        $filename .= '.pdf';
        ...
        Log::error('Error generating yearly PDF: ' . $e->getMessage());
        Log::error($e->getTraceAsString());
        return response()->json([
            'status' => 500,
            'message' => 'Erreur lors de la génération du PDF: ' . $e->getMessage()
